...
pub enum Command {
    ...
    //DEPRECATED -  MSP_IDENT                100    //out message         mixerMode + multiwii version + protocol version + capability variable
    ...
    MSP_SIMPLIFIED_TUNING = 140, //out message    Simplified tuning values and enabled state
    MSP_SET_SIMPLIFIED_TUNING = 141, //in message     Set simplified tuning positions and apply the calculated tuning
    MSP_CALCULATE_SIMPLIFIED_PID = 142, //out message    Requests calculations of PID values based on sliders. Sends the calculated values back. But don't save anything to the firmware
    MSP_CALCULATE_SIMPLIFIED_GYRO = 143, //out message    Requests calculations of gyro filter values based on sliders. Sends the calculated values back. But don't save anything to the firmware
    MSP_CALCULATE_SIMPLIFIED_DTERM = 144, //out message    Requests calculations of gyro filter values based on sliders. Sends the calculated values back. But don't save anything to the firmware
    MSP_VALIDATE_SIMPLIFIED_TUNING = 145, //out message    Returns an array of true/false showing which simpligfied tuning groups are matching with value and which are not
    ...
    MSP2_GET_OSD_WARNINGS = 0x3005, // returns active OSD warning message text
    Unknown = 0xffff,
}
...
impl From<u16> for Command {
    fn from(value: u16) -> Self {
        match value {
            1 => Command::MSP_API_VERSION,
            2 => Command::MSP_FC_VARIANT,
            3 => Command::MSP_FC_VERSION,
            4 => Command::MSP_BOARD_INFO,
            5 => Command::MSP_BUILD_INFO,
            10 => Command::MSP_NAME,
            32 => Command::MSP_BATTERY_CONFIG,
            36 => Command::MSP_FEATURE_CONFIG,
            42 => Command::MSP_MIXER_CONFIG,
            70 => Command::MSP_DATAFLASH_SUMMARY,
            ...
            160 => Command::MSP_UID,
            240 => Command::MSP_ACC_TRIM,
            246 => Command::MSP_SET_RTC,
            _ => Command::Unknown,
        }
    }
}
